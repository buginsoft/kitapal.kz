...
@endsection
@section('table_title','Книги')
@section('breadcrumb','Книги')
@section('add_button')
...
@section('table')
    @include('admin.includes.search',['base_url'=>'/admin/book'])
    <table id="showed" class="table table-bordered table-striped">
        ...
        @foreach ($book as $value)
            <tr>
                ...
                <td>
                    <form action="/admin/change-percentage/{{ $value->book_id }}" method="post">
                        @csrf
                        <input  class="form-control" style="max-width:60px;" name="percentage" value="{{$value->sale_percentage}}">
                    </form>
                </td>
                <td>
                    <a href="/admin/book/{{ $value->book_id }}/edit">
                ...
                <td>
                    <form method="post" action="/admin/archive-status">
                        ...
                        <input type="checkbox" name="status" value="1" {{$value->in_archive?'checked':''}} onChange="this.form.submit()">
                    </form>
                </td>
            </tr>
        @endforeach
