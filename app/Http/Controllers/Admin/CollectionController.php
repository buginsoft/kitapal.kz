...
class CollectionController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $collection->banner_url = $request->banner_url;

        if (!$request->has('show_badge')) {
            $collection->show_badge = 0;
        }
        if ($request->hasFile('book_image')) {
            $collection->icon = $this->saveImage($request, 'book_image');
        }
        if ($request->hasFile('banner_image')) {
            $collection->banner_image = $this->saveImage($request, 'banner_image');
        }
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($request->hasFile('banner_image')) {
            ...
            $result = $collection->banner_image;
        }
        $collection->banner_image = $result;
